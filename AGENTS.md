...
Use the checked-in `Makefile` or a local Gradle installation; a Gradle wrapper is not committed.
...
`make debug` builds the local APK. `make release` matches the CI release build environment defaults on macOS. `lint` checks Android and Compose issues. Unit and instrumented test commands are listed for future coverage; they may fail until corresponding test sources are added.
...
Do not commit release signing keys or secrets. CI expects one persistent release keystore via GitHub Actions secrets so Android upgrades remain installable across versions. Local key material is stored through `EncryptedSharedPreferences`; keep any changes to key handling explicit and reviewable.
